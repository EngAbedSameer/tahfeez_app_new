...
//   }
  ...
  Future<DocumentReference> getHalaqaDoc({required String? mEmail}) async {
    ...
    if (temp.docs.isNotEmpty) {
      log('${temp.docs.length} halaqa doc id gotten');
      log('${temp.docs.single.id} halaqa doc id gotten');
      doc = halaqa.doc(temp.docs.single.id);
    }
    return doc;
  }
  ...
  Future<DocumentReference> getStudentDoc(
    ...
    var temp = await students.where("IDn", isEqualTo: idn!.toString()).get();
  ...
  Future setStudentLastUpdate(
    ...
    log("Students last_update is up to date");
  }
  ...
  Future addMemorizer(Map<String, dynamic> data) async {
    ...
    try {
      log(' halaqat collection got ${halaqat.id} ');
      var temp = halaqat.add({
        ...
        "memorizer": {"name": data['name'], "phone": data['phone'].toString()}
          ...
          log("Added Memorizer with ID: ${documentSnapshot.id}"));
    } catch (e) {
      log('Error in adding memorizer: $e');
    }
  }
  ...
  addStudent({Map<String, dynamic>? data, mEmail}) async {
    ...
    students.add(data).then((documentSnapshot) =>
        log("Added Student with ID: ${documentSnapshot.id}"));
  }

  addRecord({Map<String, dynamic>? data, idn, mEmail}) async {
    log('''try to add record of 
      $data
      for $idn
      in memorizer $mEmail
      with data : 
    ...
    */
    ...
    try {
      records.add(data).then((record) {
        log("Added Record with ID: ${record.id}");
      });
    } catch (e) {
      log('''Error
  ...
  deleteStudent(mEmail, stdIDn) async {
    ...
    students.doc(s.docs.single.id).update({'isDeleted': 'true'}).then(
      (doc) => log("Document is deleted now"),
